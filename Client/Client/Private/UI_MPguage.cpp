...
int CMP_Guage::Tick(_float fTimeDelta)
{
	...
	if (CPlayerManager::Get_Instance()->Get_ActivePlayer()->Get_Overlimit())
	{
		m_bOverlimit = true;
		m_fCurrentOverlimitGauge = CPlayerManager::Get_Instance()->Get_ActivePlayer()->Get_Info().fCurrentOverlimitGauge;
	}
	else
		m_bOverlimit = false;
	...
	if (m_bOverlimit)
	{
		...
		m_fPosition.x = CPlayerManager::Get_Instance()->Get_ActivePlayer()->Get_ProjPosition().x - 18.f;
		m_fPosition.y = CPlayerManager::Get_Instance()->Get_ActivePlayer()->Get_ProjPosition().y + 5.f -(m_fCamDistance / 5.f);
		...
		{
			m_pTransformCom->Set_Scale(CTransform::STATE_RIGHT, 160.f / m_fCamDistance * 20.f);
			m_pTransformCom->Set_Scale(CTransform::STATE_UP, 22.f / m_fCamDistance * 20.f);
		}
		else
		{
			m_pTransformCom->Set_Scale(CTransform::STATE_RIGHT, 160.f);
...
HRESULT CMP_Guage::Render_Overlimit()
{
	if (FAILED(m_pShaderCom->Set_ShaderResourceView("g_DiffuseTexture", m_pTextureCom1->Get_SRV(0))))
		return E_FAIL;

	if (FAILED(m_pShaderCom->Set_ShaderResourceView("g_GradationTexture", m_pTextureCom2->Get_SRV(0))))
		return E_FAIL;
	...
	if (FAILED(m_pShaderCom->Set_RawValue("g_fCurrentHp", &m_fCurrentOverlimitGauge, sizeof(_float))))
		return E_FAIL;
	_float fOverlimitmax = 100.f;
	if (FAILED(m_pShaderCom->Set_RawValue("g_fMaxHp", &fOverlimitmax, sizeof(_float))))
		return E_FAIL;
...
HRESULT CMP_Guage::Ready_Components(void * pArg)
{
	...
	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture1"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_HPbar"), (CComponent**)&m_pTextureCom1)))
		return E_FAIL;

	/* For.Com_Texture */
	if (FAILED(__super::Add_Components(TEXT("Com_Texture2"), LEVEL_STATIC, TEXT("Prototype_Component_Texture_hpgrad"), (CComponent**)&m_pTextureCom2)))
		return E_FAIL;
