#include "stdafx.h"
#include "..\Public\UI_Portraitfront_sub1.h"
#include "GameInstance.h"
#include "BattleManager.h"
#include "Monster.h"
#include "UI_Dialogue.h"

CUI_Portraitfront_sub1::CUI_Portraitfront_sub1(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
	: CUI_Portraitfront(pDevice, pContext)
{
}

CUI_Portraitfront_sub1::CUI_Portraitfront_sub1(const CUI_Portraitfront_sub1 & rhs)
	: CUI_Portraitfront(rhs)
{
}

HRESULT CUI_Portraitfront_sub1::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CUI_Portraitfront_sub1::Initialize(void * pArg)
{
...
int CUI_Portraitfront_sub1::Tick(_float fTimeDelta)
{
	...
	m_fCurrentBoost = CPlayerManager::Get_Instance()->Get_EnumPlayer(4)->Get_Info().fCurrentBoostGuage;
	...
	if (m_fPosition.x >= 40.f && m_bfirst)
	{
		m_bmoveright = false;
		m_fPosition.x = 40.f;
		m_bArrived = true;
...
void CUI_Portraitfront_sub1::Late_Tick(_float fTimeDelta)
{
	if (CUI_Manager::Get_Instance()->Get_Mainmenuon())
		return;
	if (CUI_Manager::Get_Instance()->Get_StopTick() || CBattleManager::Get_Instance()->Get_IsStrike())
...
HRESULT CUI_Portraitfront_sub1::Render()
{
...
HRESULT CUI_Portraitfront_sub1::Render_Glow()
{
...
HRESULT CUI_Portraitfront_sub1::Ready_Components(void * pArg)
{
	/* For.Com_Renderer */
...
HRESULT CUI_Portraitfront_sub1::SetUp_ShaderResources()
{
	if (nullptr == m_pShaderCom)
		return E_FAIL;
...
CUI_Portraitfront_sub1 * CUI_Portraitfront_sub1::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CUI_Portraitfront_sub1*	pInstance = new CUI_Portraitfront_sub1(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CUI_Portraitfront_sub1"));
...
CGameObject * CUI_Portraitfront_sub1::Clone(void * pArg)
{
	CUI_Portraitfront_sub1*	pInstance = new CUI_Portraitfront_sub1(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CUI_Portraitfront_sub1"));
		Safe_Release(pInstance);
	}
...
void CUI_Portraitfront_sub1::Free()
{
