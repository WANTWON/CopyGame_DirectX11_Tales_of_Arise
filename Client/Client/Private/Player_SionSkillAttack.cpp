		...
		if (pEvent.isPlay)
		{
				...
				if (ANIMEVENT::EVENTTYPE::EVENT_COLLIDER == pEvent.eType)
				{
					if ((m_fEventStart != pEvent.fStartTime))
					{
						if (m_pOwner->Get_Model()->Get_CurrentAnimIndex() == (CSion::ANIM::BTL_ATTACK_MAGNARAY))
						{
							...
							CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_E.wav"), SOUND_SKILL, 0.5f);
						}
						...
						else if (m_pOwner->Get_Model()->Get_CurrentAnimIndex() == (CSion::ANIM::BTL_ATTACK_TRESVENTOS))
						{
							...
							m_fEventStart = pEvent.fStartTime;
							m_iCount++;
							CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_Jump_E_TEST.wav"), SOUND_SKILL, 0.5f);
						}
					}
				}
			...
			case Client::CPlayerState::STATE_SKILL_ATTACK_R:
				if (ANIMEVENT::EVENTTYPE::EVENT_COLLIDER == pEvent.eType && !m_bBulletMade)
				{
					...
					{
						CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_R_Test.wav"), SOUND_SKILL, 0.5f);
						CBaseObj * pTarget = CBattleManager::Get_Instance()->Get_LackonMonster();
						if (pTarget == nullptr)
							pTarget = dynamic_cast<CMonster*>(CBattleManager::Get_Instance()->Get_MinDistance_Monster(m_pOwner->Get_Transform()->Get_State(CTransform::STATE_TRANSLATION)));
			...
			case Client::CPlayerState::STATE_SKILL_ATTACK_F:
				if (ANIMEVENT::EVENTTYPE::EVENT_COLLIDER == pEvent.eType)
				{
					...
					{
						CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_F.wav"), SOUND_SKILL, 0.8f);
						CBaseObj * pTarget = CBattleManager::Get_Instance()->Get_LackonMonster();
						if (pTarget == nullptr)
							pTarget = dynamic_cast<CMonster*>(CBattleManager::Get_Instance()->Get_MinDistance_Monster(m_pOwner->Get_Transform()->Get_State(CTransform::STATE_TRANSLATION)));
			...
			case Client::CPlayerState::STATE_SKILL_ATTACK4:
				if (ANIMEVENT::EVENTTYPE::EVENT_COLLIDER == pEvent.eType)
				{
					if ((m_fEventStart != pEvent.fStartTime))
					{
						CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_Ctrl_E.wav"), SOUND_SKILL, 0.6f);
						CBaseObj * pTarget = CBattleManager::Get_Instance()->Get_LackonMonster();
						if (pTarget == nullptr)
							pTarget = dynamic_cast<CMonster*>(CBattleManager::Get_Instance()->Get_MinDistance_Monster(m_pOwner->Get_Transform()->Get_State(CTransform::STATE_TRANSLATION)));
			...
			case Client::CPlayerState::STATE_SKILL_ATTACK5:
				if (ANIMEVENT::EVENTTYPE::EVENT_COLLIDER == pEvent.eType && !m_bBulletMade)
				{
					...
					if ((m_fEventStart != pEvent.fStartTime))
					{
						CGameInstance::Get_Instance()->PlaySounds(TEXT("SionSkillSound_Ctrl_R_Test.wav"), SOUND_SKILL, 0.6f);
						CBaseObj * pTarget = CBattleManager::Get_Instance()->Get_LackonMonster();
						if (pTarget == nullptr)
							pTarget = dynamic_cast<CMonster*>(CBattleManager::Get_Instance()->Get_MinDistance_Monster(m_pOwner->Get_Transform()->Get_State(CTransform::STATE_TRANSLATION)));
