...
CKisara::CKisara(const CKisara & rhs)
	: CPlayer(rhs)
{
}

HRESULT CKisara::Initialize_Prototype(void)
{
	return S_OK;
}

HRESULT CKisara::Initialize(void * pArg)
{
	m_ePlayerID = KISARA;
...
_bool CKisara::Is_AnimationLoop(_uint eAnimId)
{
	switch ((ANIM)eAnimId)
	{
...
HRESULT CKisara::Ready_Components(void * pArg)
{
	/* For.Com_Renderer */
	...
	/* For.Com_Model*/
	if (FAILED(__super::Add_Components(TEXT("Com_Model"), LEVEL_STATIC, TEXT("Kisara"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CKisara::SetUp_ShaderResources(void)
{
...
CKisara * CKisara::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pContext)
{
	CKisara* pInstance = new CKisara(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		ERR_MSG(TEXT("Failed to Created : CKisara"));
...
CGameObject * CKisara::Clone(void * pArg)
{
	CKisara* pInstance = new CKisara(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		ERR_MSG(TEXT("Failed to Cloned : CKisara"));
		Safe_Release(pInstance);
	}
