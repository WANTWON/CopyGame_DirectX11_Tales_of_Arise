...
int CWeapon::Tick(_float fTimeDelta)
{
	...
	if (m_isRotation)
	{
		...
		_matrix		RotationMatrix = XMMatrixRotationAxis(m_WeaponDesc.pSocket->Get_CombinedTransformationMatrix().r[0], XMConvertToRadians(90.f) * m_fRotateTime * 10.f);
...
void CWeapon::Late_Tick(_float fTimeDelta)
{
	if (nullptr != m_pSPHERECom)
	{
		...
		if (CCollision_Manager::Get_Instance()->CollisionwithGroup(CCollision_Manager::COLLISION_MONSTER, m_pSPHERECom, &pCollisionTarget))
		{
			...
			if (pCollided)
			{
				pCollided->Take_Damage(rand() % 100, m_WeaponDesc.pOwner, m_HitLagDesc);
			}
