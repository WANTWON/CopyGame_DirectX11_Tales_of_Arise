...
HRESULT CCamera_Manager::Add_Camera()
{
	...
	XMStoreFloat3(&NonAnimDesc.vPosition, pTransform->Get_State(CTransform::STATE_POSITION));
...
void CCamera_Manager::Update_CamPosition(_float3 position)
{
	if (m_vecCameras.size() != 0)
	{
		...
		m_vecCameras[m_vecCameras.size() - 1]->Set_State(CTransform::STATE_POSITION, vPosition);
	}
}
